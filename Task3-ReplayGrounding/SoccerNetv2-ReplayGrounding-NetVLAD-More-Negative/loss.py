...
class ReplayGroundingSpottingLoss(torch.nn.Module):
    ...
    def forward(self,y_true, y_pred):
        ...
        # loss = torch.sum(y_true[:,:,0]*self.lambda_coord*torch.square(y_true[:,:,1]-y_pred[:,:,1])  +  y_true[:,:,0]*torch.square(y_true[:,:,0]-y_pred[:,:,0]) +  (1-y_true[:,:,0])*self.lambda_noobj*torch.square(torch.max((y_true[:,:,0]-y_pred[:,:,0]-self.alfa),torch.zeros((y_true.shape[0],1)).cuda())))
        ...
        # loss = torch.sum(y_true[:,:,0]*torch.square(y_true[:,:,0]-y_pred[:,:,0]) +  (1-y_true[:,:,0])*self.lambda_noobj*torch.square(y_true[:,:,0]-y_pred[:,:,0])) #-y_true[:,:,0]*torch.sum(y_true[:,:,2:]*torch.log(y_pred[:,:,2:]),axis=-1)
