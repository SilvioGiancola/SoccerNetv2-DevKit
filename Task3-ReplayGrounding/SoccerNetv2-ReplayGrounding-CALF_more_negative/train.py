...
def train(dataloader,
    ...
    with tqdm(enumerate(dataloader), total=len(dataloader), ncols=160) as t:
        for i, (feats_all, labels_all, targets_all) in t: 
            if np.random.randint(10, size=1)<5:
                 continue
            # measure data loading time
            data_time.update(time.time() - end)
            for (feats, labels, targets) in zip(feats_all, labels_all, targets_all):
                feats = feats.cuda()
...
def test(dataloader,model, model_name,split,annotation_path,detection_path,save_results):
    ...
    with tqdm(enumerate(dataloader), total=len(dataloader), ncols=120) as t:
        ...
        if save_results:
            ...
            # gt_list,detections_list=replay_from_json(annotation_path,detection_path,split,model.framerate)
            # a_mAP_json = average_mAP(gt_list, detections_list,  model.framerate)
