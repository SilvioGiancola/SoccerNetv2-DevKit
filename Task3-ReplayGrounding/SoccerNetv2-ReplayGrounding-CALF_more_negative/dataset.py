...
class SoccerNetReplayClips(Dataset):
    def __init__(self, path, features="ResNET_PCA512.npy", split="train", 
                framerate=2, chunk_size=240, receptive_field=80, chunks_per_epoch=6000, loop=5):
        self.path = path
    ...
    def __getitem__(self, index):
        ...
        for iloop in np.arange(self.loop):
            ...
            if iloop in np.arange(1,self.loop):
                ...
                if len(list_events) > 0 and iloop in np.arange(1,int(self.loop/2)):
            ...
            clip_stacked_all.append( torch.from_numpy(clip_stacked))
            clip_TSE_all.append(torch.from_numpy(np.expand_dims(clip_TSE, axis=-1)))
            clip_target_all.append(torch.from_numpy(clip_target))
            # replay_mask_all.append(torch.from_numpy(replay_mask))

        # return torch.from_numpy(clip_stacked_all), torch.from_numpy(np.expand_dims(clip_TSE, axis=-1)), torch.from_numpy(clip_target)
        return clip_stacked_all, clip_TSE_all, clip_target_all
