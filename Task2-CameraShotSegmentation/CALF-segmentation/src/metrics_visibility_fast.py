...
def compute_precision_recall_curve(targets, closests, detections, delta, NMS_on):
    ...
    for c in np.arange(num_classes):
        ...
        for target, closest, detection in zip(targets, closests, detections_NMS):
...
def delta_curve(targets, closests, detections,  framerate, NMS_on):
...
def average_mAP_visibility(targets, detections, framerate=2, NMS_on=True):
    ...
    mAP, mAP_per_class, mAP_visible, mAP_per_class_visible, mAP_unshown, mAP_per_class_unshown = delta_curve(targets_numpy, closests_numpy, detections_numpy, framerate, NMS_on)
