...
class Model(nn.Module):
    def __init__(self, weights=None, input_size=512, num_classes_type=13,num_classes_change=1, chunk_size=240, dim_capsule=16, receptive_field=80, num_detections=5, framerate=2):
        ...
        k = 10
        self.conv_1 = nn.Conv1d(in_channels=input_size, out_channels=128, kernel_size=2*k+1, padding=k)
        self.conv_2 = nn.Conv1d(in_channels=128, out_channels=32, kernel_size=2*k+1, padding=k)
        self.conv_3 = nn.Conv1d(in_channels=32, out_channels=self.num_classes_type, kernel_size=2*k+1, padding=k)


        # self.conv2d_1 = nn.Conv2d(in_channels=1, out_channels=128, kernel_size=(1,input_size))
        # self.conv2d_2 = nn.Conv2d(in_channels=128, out_channels=32, kernel_size=(1,1))
        # self.conv2d_3 = nn.Conv2d(in_channels=32, out_channels=14, kernel_size=(1,1))
    ...
    def forward(self, inputs):
        ...
        # print("Conv_3 size: ", conv_3.size())


        # conf_pred = torch.sigmoid(conv_3)
        # print("conf_pred size: ", conf_pred.size())
        segmentation_output = torch.softmax(conv_3, dim=1)
        # print("segmentation_output size: ", segmentation_output.size())
        ...
        # # Base Convolutional Layers
        # conv_1 = F.relu(self.conv2d_1(inputs))
        # #print("Conv_1 size: ", conv_1.size())
        
        # conv_2 = F.relu(self.conv2d_2(conv_1))
        # # print("Conv_2 size: ", conv_2.size())
