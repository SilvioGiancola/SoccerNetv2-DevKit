...
def main(args, model_save_path):
    ...
    if not args.test_only:
        dataset_Train = SoccerNetClipsTesting(path=args.SoccerNet_path, features=args.features, split="train", version=args.version, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate)
        dataset_Valid = SoccerNetClipsTesting(path=args.SoccerNet_path, features=args.features, split="valid", version=args.version, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate)
    ...
    # create model  
    model = Model(weights=args.load_weights, chunk_size=args.chunk_size*args.framerate, dim_capsule=args.dim_capsule, receptive_field=args.receptive_field*args.framerate, num_detections=dataset_Train.num_detections, framerate=args.framerate).cuda()
    ...
    if not args.test_only:
        ...
        if args.scheduler == "ExponentialDecay":
            ...
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', verbose=True, patience=args.patience)

        # start training
        trainer(train_loader, val_loader, val_metric_loader,test_loader, 
                model, optimizer, scheduler, criterion, [args.loss_weight_segmentation, args.loss_weight_detection],
...
if __name__ == '__main__':
    ...
    parser.add_argument("--criterion", required=False, type=str, default="MSE", help="define loss [CE/MSE]")
    ...
    log_path = os.path.join(model_save_path, f"{start_time}.txt")
